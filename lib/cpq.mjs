...
export const processCpqLog = (lineAt, lineCount, renderTab, log) => {

    //Advanced Rules
    var entityFilterCount = 0;
    var itemExistsCheck = 0;
    var addRules = 0, deleteRules = 0, modifyRules = 0, replaceRules = 0, reqExcRecRules = 0;
    //Context Rules
    var ctxRules = 0;
    //Response
    var productLog = '';
    ...
    try {
        ...
        for(let i=0; i< lineCount; i++) {
            ...
            var line = lineAt(i);
            if(line.includes('ValidationRulesImplementation.validateLineItems') && line.includes('METHOD_ENTRY')) {
            
                while(i++<lineCount && !(line.includes('ValidationRulesImplementation.validateLineItems') && line.includes('METHOD_EXIT'))) {
                    line = lineAt(i);
                    if(line.includes('CompiledFilterEvaluator.evaluate') && line.includes('METHOD_ENTRY')) {
                        entityFilterCount++;
                    }
                ...
                while(i++<lineCount && !(line.includes('CtxRuleProcessor.processRules') && line.includes('METHOD_EXIT'))) {
                    line = lineAt(i);
                    if(line.includes('CtxRuleSetProcessor.evaluateRuleSetRules') && line.includes('METHOD_ENTRY')) {
                        ctxRules++;
                    }
                }
                ...
                while(i++<lineCount && !(line.includes('V2ProductHierarchyService.mergeHierarchyAndItems') && line.includes('METHOD_EXIT'))) {
                    ...
                    if(line.includes('V2ProductHierarchyService.populateRecordFields') && line.includes('METHOD_ENTRY')) {
                        var attribCount = 0, fieldCount = 0;
                        while(i++<lineCount && !(line.includes('V2ProductHierarchyService.populateRecordFields') && line.includes('METHOD_EXIT'))) {
                            line = lineAt(i);
                            if((line.includes('AttributeBaseService.parseAttributesObject') || line.includes('JSONAttributeUtilities.toJSONAttribute')) && line.includes("METHOD_ENTRY")) {
                                attribCount++;
                            }
                        ...
                        productLog += 'Product:<br>&emsp;Attributes:'+attribCount+'<br>&emsp;Fields: '+fieldCount+'<br><br>';
                    }
                    if(line.includes('V2ProductHierarchyService.populateLineItemRecordFields') && line.includes('METHOD_ENTRY')) {
                        ...
                        productLog += 'LineItem:<br>&emsp;Attributes:'+attribCount+'<br>&emsp;Fields: '+fieldCount+'<br><br>';
                    }
                    if(line.includes('V2ProductHierarchyService.populateVirtualItemRecordFields') && line.includes('METHOD_ENTRY')) {
                        ...
                        productLog += 'VirtualLineItem:<br>&emsp;Attributes:'+attribCount+'<br>&emsp;Fields: '+fieldCount+'<br><br>';
                    }
                }
            }
        }
