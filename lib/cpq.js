...
function processCpqLog(lineAt, lineCount, renderTab, log) {
    ...
    var entityFilters = [], entityFilterText='';
    var actions = [];
    var ruleListText = [];
    // Pricing
                        ...
                        while(i++<lineCount && !((line.includes('ConfigRuleHandler.validateItems')) && line.includes('METHOD_EXIT'))) {
                            ...
                            if(line.includes('|rule|{')) {
                                ...
                                if(ruleStr != "{}") {
                                    ...
                                    let ruleText = 'Rule Name: "'+rule.Name + '"<ul>';
                                    rule.vlocity_cmt__RuleFilters__r.forEach(filter => {
                                        ruleText += '<li> Filter: "'+filter.vlocity_cmt__EntityFilterId__r.Name + '" </li>';
                                    });
                                    rule.vlocity_cmt__RuleActions__r.forEach(action => {
                                        try {
                                            ruleText += '<li> Action: ('+action.vlocity_cmt__ProductRelationshipId__r.vlocity_cmt__RelationshipType__c + ') -> "' +
                                            action.vlocity_cmt__ProductRelationshipId__r.Name + '"</li>';
                                        } catch(exc) {}
                                    });
                                    ruleText += '</ul><br>\n';
                                    ruleListText += ruleText;
                                }
                            }
